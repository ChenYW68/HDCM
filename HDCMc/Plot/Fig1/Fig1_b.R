...
Site_label <- plyr::ddply(Site[, c("CITY", "LON", "LAT")],
                          .(CITY),
                          plyr::summarize,
                          LON = mean(LON),
                          LAT  =  mean(LAT),
...
###################################################################
{
  p1 <- ggmap::ggmap(GoogleMap_BTH) +
    ...
    geom_point(data = CMAQ_Site , aes(x = LON, y = LAT)
               , shape = 20, col ="gray50", size = 0.3) +
    geom_point(data = Site, aes(x = LON, y = LAT)##A9A9A9
               , shape = 20
               , col = "red", size = 1) +
...
ggsave(plot = p1, height = 6, width = 5, dpi = 400,
       file = paste0(file, '/Fig1_b.png'))
