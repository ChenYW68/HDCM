...
PM25_2015s <- ADCM::spCoords.transform(obs_PM25_2015s[, -INDX],
                                           method = 1)
...
Nt <- length(unique(PM25_2015s$time.index))
region <- sort(as.character(unique(PM25_2015s$CITY)))
...
setDT(PM25_2015s)
for(r in region_num){#region_num
  cat(paste0("\n\n   The ", r, "th region: ", region[r], "!!!\n\n"))
  Da.mod <- PM25_2015s[CITY %nin% region[r], ]
  Da.pre <- PM25_2015s[CITY %in% region[r], ]
  colnames(Da.mod)
  ...
  Pred.sd <- as.vector(Pred.sd)*2*pred.mean
  Da.pre$REAL_PM25 <- Da.pre$REAL_PM25^2
  
  PM25.Pred <- ifelse(PM25.Pred < 0, 0, PM25.Pred^2)#
  
  
  PM25.U95 <- PM25.Pred + 1.96*Pred.sd#ifelse(PM25.U95 < 0, 0, PM25.U95)#^2
  PM25.L25 <- PM25.Pred - 1.96*Pred.sd#ifelse(PM25.L25 < 0, 0, PM25.L25)#^2
...
temp$Coverage <- round(ifelse(is.na(temp$Coverage), 
                              ...
                              temp$Coverage), 4)
temp
# writexl::write_xlsx(temp[, c(1:3, 6, 5)], path = "./Result/orig_INLAs_cv.xlsx")
# writexl::write_xlsx(Inla, path = "./Result/orig_pred_INLAs_cv.xlsx")
writexl::write_xlsx(temp[, c(1:3, 6, 5)], path = "./Result/INLAs_cv.xlsx")
writexl::write_xlsx(Inla, path = "./Result/pred_INLAs_cv.xlsx")
