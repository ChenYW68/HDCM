...
setDT(PM25_2015s)
{
  ...
  {
    set.seed(1234)
    year_range <- unique(PM25_2015s$YEAR)
    # Matrix with the estimated daily covariance parameters
    cov.pars.daily <- NULL
    for(Year in year_range)
    {
      Base_Table <- PM25_2015s[YEAR == Year,]
    ...
    for(Year in year_range)
    {
      ...
      for(Month in month_range)
      {
        ...
        for(Day in day_range)
        {
          ...
          PM25.Pred.sd <- sqrt(as.numeric(pred.uk.day$krige.var))* 2*testPred
...
#                               temp$INS), 4)
temp
# writexl::write_xlsx(temp, path = "./Result/orig_UKs_cv.xlsx")
# writexl::write_xlsx(UK, path = "./Result/orig_pred_UKs_cv.xlsx")
writexl::write_xlsx(temp[, c(1:3, 6, 5)], path = "./Result/UKs_cv.xlsx")
writexl::write_xlsx(UK, path = "./Result/pred_UKs_cv.xlsx")
