...
Cs <- 5e-1
Obj.Seq <- 1:7
theta.2 <- c(1e-2, 1e-3, 1)
res.num <- 1
para <- list(# beta = list(E_beta = c(10, 2.5, rep(0, p1 - 2))), # 6, 2
  obs.sigma.sq = list(E_sigma.sq = 1, a = 2, b = 1)
  , theta.1 = list(E_theta.1 = rep(1e-1, res.num))
  , theta.2 = list(E_theta.2 = rep(theta.2[1], res.num))
  , zeta = list(E_zeta = rep(5e0, res.num))
  , zeta0 = list(E_zeta0 = rep(5e0, res.num))
  , proc.tau.sq = list(E_tau.sq = rep(1e0, res.num))
...
##########################################################################################
{
  ...
  prior <- list(
    ...
    , zeta = list(a = rep(1e-3, res.num), b =  rep(2e2, res.num))
    , zeta0 = list(a = rep(1e-3, res.num), b =  rep(2e2, res.num))
...
# mg <- nrow(G)
# dx <- as.vector(H.basic.data$Grid.infor$level[[g]]$BAUs.Dist*G)
