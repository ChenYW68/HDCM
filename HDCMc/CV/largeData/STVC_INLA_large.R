...
for(r in 1){
  ...
  formula <- (PM25 ~ -1 + Intercept + sim_CMAQ_PM25 +
                TEMP + WIND_X +
                WIND_Y +
                f(time.index, sim_CMAQ_PM25, constr = TRUE,
...
Inla$Run_time <- (end.time - start.time)[3]
writexl::write_xlsx(temp0, path = paste0("./Result/STVCxz_varyingBeta1_INLAw_large_",
                                         mesh$n,"_cv.xlsx"))
writexl::write_xlsx(Inla, path = paste0("./Result/STVCxz_varyingBeta1_pred_INLAw_large_",
                                        mesh$n,"_cv.xlsx"))
