...
set.seed(12345)
test.id <- sample(Site$ID, 2000, replace = F)
cat("The number of training set:", nrow(Site) - length(test.id))



Site$Flag <- ifelse(Site$ID %in% test.id, "test", "train")
temp <- Site[Site$Flag == "test", ]
plot(temp$LON, temp$LAT)
# NAQPMS_CMAQ_Dataset_2015W$YEAR_MONTH <- ifelse(NAQPMS_CMAQ_Dataset_2015W$YEAR_MONTH == 20161,
#                                                201601, 
#                                                NAQPMS_CMAQ_Dataset_2015W$YEAR_MONTH)
# save(Site, NAQPMS_CMAQ_Dataset_2015W, file =  "./data/NAQPMS_CMAQ_Dataset_2015W.RData")

PM25_2015w <- NAQPMS_CMAQ_Dataset_2015W[distToNearestCAMQpoint <= 15]
PM25_2015w$Flag <- ifelse(PM25_2015w$ID %in% test.id, "test", "train")
...
range(H.basic.data$Grid.infor$summary$Hdist)*0.01
...
##########################################################################################
{
  ...
  para <- list( beta = list(E_beta = c(8, 1, rep(0, p1 - 2))),
                ...
                , proc.tau.sq = list(E_tau.sq = rep(1e1, res.num))
...
CV_T_Dist_W <- HDCM(Tab = paste0("HDCM_ANA1_", tab),
                    Site = Site,
                    HDCM.Data = ADCM.Data,
